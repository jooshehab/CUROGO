'use client'

import { ChevronLeft, MapPin, Clock, Phone, ArrowLeft, Building2, MessageCircle } from 'lucide-react'
import { motion, useReducedMotion } from 'framer-motion'
import { CITIES, SERVICES, SITE } from '@/lib/site-data'
import { useHashRoute, buildHref } from '@/hooks/use-hash-route'
import { CTABanner } from '@/components/site/sections/cta-banner'
import { getServiceImage } from '@/lib/service-images'

export function CityPage({ slug }: { slug: string }) {
  const { navigate } = useHashRoute()
  const prefersReducedMotion = useReducedMotion()
  const city = CITIES.find((c) => c.slug === slug)

  const handleNav = (e: React.MouseEvent, href: string) => {
    e.preventDefault()
    navigate(href)
  }

  if (!city) {
    return (
      <div className="container-curego py-20 text-center">
        <h1 className="text-3xl font-extrabold mb-3">المدينة غير موجودة</h1>
        <a href={buildHref('/areas')} onClick={(e) => handleNav(e, '/areas')}
          className="inline-flex items-center gap-2 px-6 py-3 rounded-xl gradient-emergency text-white font-bold">
          <ArrowLeft className="h-4 w-4" /> كل المناطق
        </a>
      </div>
    )
  }

  const otherCity = CITIES.find(c => c.slug !== city.slug)

  return (
    <>
      <section className="bg-grid-pattern py-12 lg:py-16">
        <div className="container-curego">
          <nav className="text-sm text-muted-foreground mb-4 flex items-center gap-1.5 flex-wrap" aria-label="مسار التنقل">
            <a href="/" onClick={(e) => handleNav(e, '/')} className="hover:text-primary">الرئيسية</a>
            <ChevronLeft className="h-4 w-4" />
            <a href={buildHref('/areas')} onClick={(e) => handleNav(e, '/areas')} className="hover:text-primary">المناطق</a>
            <ChevronLeft className="h-4 w-4" />
            <span className="text-foreground font-bold">{city.name}</span>
          </nav>

          <div className="grid lg:grid-cols-3 gap-8 items-start">
            <motion.div
              initial={prefersReducedMotion ? false : { opacity: 0, y: 20 }}
              animate={prefersReducedMotion ? {} : { opacity: 1, y: 0 }}
              transition={{ duration: 0.6 }}
              className="lg:col-span-2"
            >
              <div className="flex items-start gap-4 mb-6">
                <span className="inline-flex h-16 w-16 items-center justify-center rounded-2xl gradient-emergency text-white shadow-emergency shrink-0">
                  <Building2 className="h-8 w-8" />
                </span>
                <div>
                  <h1 className="text-3xl lg:text-5xl font-extrabold mb-2">خدماتنا في {city.name}</h1>
                  <p className="text-muted-foreground text-base lg:text-lg">
                    بنوفر كل خدماتنا الطبية المنزلية في {city.name} · {city.districts.length} منطقة مغطاة · استجابة سريعة 24 ساعة
                  </p>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-3">
                <a
                  href={`tel:${SITE.phoneIntl}`}
                  className="inline-flex items-center gap-2 px-5 py-3 rounded-xl gradient-emergency text-white font-bold shadow-emergency hover:opacity-95 transition-opacity"
                  dir="ltr"
                >
                  <Phone className="h-4 w-4" />
                  اتصل من {city.name}
                </a>
                <a
                  href={buildHref('/booking')}
                  onClick={(e) => handleNav(e, '/booking')}
                  className="inline-flex items-center gap-2 px-5 py-3 rounded-xl border-2 border-primary text-primary font-bold hover:bg-primary/5 transition-colors"
                >
                  احجز خدمة في {city.name}
                </a>
                <a
                  href={`https://wa.me/${SITE.whatsapp}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-success/10 text-success font-bold hover:bg-success/20 transition-colors"
                >
                  <MessageCircle className="h-4 w-4" />
                  واتساب
                </a>
              </div>
            </motion.div>

            <motion.div
              initial={prefersReducedMotion ? false : { opacity: 0, x: 20 }}
              animate={prefersReducedMotion ? {} : { opacity: 1, x: 0 }}
              transition={{ duration: 0.6, delay: 0.2 }}
              className="bg-card rounded-2xl shadow-card p-6"
            >
              <h3 className="font-extrabold text-lg mb-4">إحصائيات {city.name}</h3>
              <div className="grid grid-cols-2 gap-3 text-center">
                <div className="p-3 rounded-lg bg-muted/30">
                  <div className="text-2xl font-extrabold text-primary">{city.districts.length}</div>
                  <div className="text-xs text-muted-foreground">منطقة مغطاة</div>
                </div>
                <div className="p-3 rounded-lg bg-muted/30">
                  <div className="text-2xl font-extrabold text-primary">{city.districts[0].responseMin}-{city.districts[city.districts.length-1].responseMin}</div>
                  <div className="text-xs text-muted-foreground">دقيقة استجابة</div>
                </div>
                <div className="p-3 rounded-lg bg-muted/30">
                  <div className="text-2xl font-extrabold text-primary">24/7</div>
                  <div className="text-xs text-muted-foreground">ساعات العمل</div>
                </div>
                <div className="p-3 rounded-lg bg-muted/30">
                  <div className="text-2xl font-extrabold text-primary">{SERVICES.length}</div>
                  <div className="text-xs text-muted-foreground">خدمة متاحة</div>
                </div>
              </div>
            </motion.div>
          </div>
        </div>
      </section>

      {/* Description */}
      <section className="py-12 lg:py-16">
        <div className="container-curego">
          <div className="max-w-3xl mx-auto lg:mx-0">
            <motion.h2
              initial={prefersReducedMotion ? false : { opacity: 0, y: 20 }}
              whileInView={prefersReducedMotion ? {} : { opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="text-2xl lg:text-3xl font-extrabold mb-5"
            >
              عن تغطيتنا في {city.name}
            </motion.h2>
            <div className="space-y-4 text-base text-foreground/90 leading-relaxed">
              {city.description.map((p, i) => (
                <motion.p
                  key={i}
                  initial={prefersReducedMotion ? false : { opacity: 0, y: 20 }}
                  whileInView={prefersReducedMotion ? {} : { opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: i * 0.1 }}
                >
                  {p}
                </motion.p>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* Districts grid — كل منطقة في صف منفصل */}
      <section className="py-12 lg:py-16 bg-muted/30">
        <div className="container-curego">
          <motion.h2
            initial={prefersReducedMotion ? false : { opacity: 0, y: 20 }}
            whileInView={prefersReducedMotion ? {} : { opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-2xl lg:text-3xl font-extrabold mb-2 text-center"
          >
            مناطق {city.name} ({city.districts.length})
          </motion.h2>
          <p className="text-muted-foreground text-center mb-10">اضغط على منطقتك لمعرفة تفاصيل التغطية فيها</p>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {city.districts.map((district, dIdx) => (
              <motion.a
                key={district.slug}
                href={buildHref(`/areas/${city.slug}/${district.slug}`)}
                onClick={(e) => handleNav(e, `/areas/${city.slug}/${district.slug}`)}
                initial={prefersReducedMotion ? false : { opacity: 0, y: 24 }}
                whileInView={prefersReducedMotion ? {} : { opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.2 }}
                transition={{ duration: 0.4, delay: (dIdx % 3) * 0.08 }}
                whileHover={prefersReducedMotion ? {} : { y: -6 }}
                className="group bg-card rounded-2xl p-5 shadow-card border border-border/50 hover:border-primary/30 hover:shadow-card-hover transition-all"
              >
                <div className="flex items-start gap-3 mb-3">
                  <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary group-hover:gradient-emergency group-hover:text-white transition-all shrink-0">
                    <MapPin className="h-5 w-5" />
                  </span>
                  <div className="flex-1">
                    <h3 className="font-extrabold text-lg mb-1">{district.name}</h3>
                    <div className="text-xs text-primary font-bold flex items-center gap-1">
                      <Clock className="h-3 w-3" />
                      استجابة {district.responseMin} دقيقة
                    </div>
                  </div>
                </div>
                <p className="text-sm text-muted-foreground leading-relaxed">{district.shortDesc}</p>
                <div className="mt-3 pt-3 border-t border-border/50 flex flex-wrap gap-1">
                  {district.landmarks.slice(0, 3).map((lm, i) => (
                    <span key={i} className="text-[10px] px-2 py-0.5 rounded-full bg-muted/50 text-muted-foreground">
                      {lm}
                    </span>
                  ))}
                </div>
                <div className="mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-primary group-hover:gap-3 transition-all">
                  تفاصيل التغطية
                  <ArrowLeft className="h-3.5 w-3.5" />
                </div>
              </motion.a>
            ))}
          </div>
        </div>
      </section>

      {/* Services in this city */}
      <section className="py-12 lg:py-16">
        <div className="container-curego">
          <motion.h2
            initial={prefersReducedMotion ? false : { opacity: 0, y: 20 }}
            whileInView={prefersReducedMotion ? {} : { opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="text-2xl lg:text-3xl font-extrabold mb-2 text-center"
          >
            خدماتنا في {city.name} ({SERVICES.length})
          </motion.h2>
          <p className="text-muted-foreground text-center mb-10">كل خدماتنا الطبية المنزلية متوفرة في {city.name}</p>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {SERVICES.map((s, idx) => (
              <motion.a
                key={s.slug}
                href={buildHref(`/services/${s.slug}`)}
                onClick={(e) => handleNav(e, `/services/${s.slug}`)}
                initial={prefersReducedMotion ? false : { opacity: 0, y: 20 }}
                whileInView={prefersReducedMotion ? {} : { opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: idx * 0.04 }}
                whileHover={prefersReducedMotion ? {} : { y: -4 }}
                className="group bg-card rounded-2xl overflow-hidden shadow-card border border-border/50 hover:border-primary/30 hover:shadow-card-hover transition-all"
              >
                <div className="relative aspect-[16/9] overflow-hidden bg-muted/30">
                  <img
                    src={getServiceImage(s.slug, 'sm')}
                    srcSet={`${getServiceImage(s.slug, 'sm')} 400w, ${getServiceImage(s.slug, 'md')} 800w`}
                    sizes="(max-width: 640px) 50vw, 25vw"
                    alt={s.name}
                    className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                    loading="lazy"
                    decoding="async"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-card/90 to-transparent" />
                </div>
                <div className="p-4">
                  <h3 className="font-extrabold text-base mb-1 group-hover:text-primary transition-colors">{s.name}</h3>
                  <p className="text-xs text-muted-foreground line-clamp-2">{s.shortDesc}</p>
                </div>
              </motion.a>
            ))}
          </div>
        </div>
      </section>

      {/* Other city link */}
      {otherCity && (
        <section className="py-8 pb-12">
          <div className="container-curego">
            <motion.a
              href={buildHref(`/areas/${otherCity.slug}`)}
              onClick={(e) => handleNav(e, `/areas/${otherCity.slug}`)}
              initial={prefersReducedMotion ? false : { opacity: 0, y: 20 }}
              whileInView={prefersReducedMotion ? {} : { opacity: 1, y: 0 }}
              viewport={{ once: true }}
              whileHover={prefersReducedMotion ? {} : { y: -4 }}
              className="block bg-muted/30 rounded-2xl p-6 hover:bg-muted/50 transition-colors"
            >
              <div className="flex items-center justify-between gap-4">
                <div>
                  <div className="text-xs text-muted-foreground mb-1">المدينة التانية</div>
                  <div className="text-xl font-extrabold flex items-center gap-2">
                    <Building2 className="h-5 w-5 text-primary" />
                    خدماتنا في {otherCity.name} ({otherCity.districts.length} منطقة)
                  </div>
                </div>
                <ArrowLeft className="h-6 w-6 text-primary" />
              </div>
            </motion.a>
          </div>
        </section>
      )}

      <CTABanner />
    </>
  )
}
